...
from phc.learning.network_loader import load_z_encoder, load_z_decoder, load_z_left_decoder, load_z_right_decoder
...
class HumanoidZ(humanoid.Humanoid):
    ...
    def initialize_z_models(self):
        ...
        if self.use_part:
            ...
            self.left_decoder = load_z_left_decoder(check_points[0], activation = self.z_activation, z_type = self.distill_z_type, device = self.device) 
            self.right_decoder = load_z_right_decoder(check_points[0], activation = self.z_activation, z_type = self.distill_z_type, device = self.device) 
    ...
    def compute_z_actions(self, action_z):
        with torch.no_grad():
            ...
            if self.distill_z_type == "vq_vae":
                ...
                if self.use_vae_prior:
                    if self.use_part:
                        ...
                        z_prior_mu = self.decoder.z_prior_mu(z_prior_out)
                        z_prior_out_left = self.left_decoder.z_prior(self_obs[:, self.self_obs_left_hand_idxes])
                        z_prior_mu_left = self.left_decoder.z_prior_mu(z_prior_out_left)
                        z_prior_out_right = self.right_decoder.z_prior(self_obs[:, self.self_obs_right_hand_idxes])
                        z_prior_mu_right = self.right_decoder.z_prior_mu(z_prior_out_right)
                        action_z = torch.cat([z_prior_mu, z_prior_mu_left, z_prior_mu_right], dim = -1) + action_z
            ...
            if self.z_all:
                ...
                if self.use_part:
                    x_all = torch.zeros([action_z.shape[0], len(self._dof_names) * 3], device = action_z.device)
                    x_body = self.decoder.decoder(torch.cat([self_obs, action_z], dim = -1))
                    x_left = self.left_decoder.decoder(torch.cat([self_obs[:, self.self_obs_left_hand_idxes], action_z[:, int(self.embedding_size_distill * 4/6):(int(self.embedding_size_distill * 4/6) + int(self.embedding_size_distill * 1/6)) ]], dim = -1))
                    x_right = self.right_decoder.decoder(torch.cat([self_obs[:, self.self_obs_right_hand_idxes], action_z[:, int(self.embedding_size_distill * 5/6):]], dim = -1))
                    x_all[:, self.body_name_idxes] = x_body
                    x_all[:, self.left_dof_idxes] = x_left
                    x_all[:, self.right_dof_idxes] = x_right
                else:
