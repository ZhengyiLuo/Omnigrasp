...
class HumanoidIm(humanoid_amp_task.HumanoidAMPTask):
    ...
    def get_task_obs_size(self):
        ...
        if (self._enable_task_obs):
            if self.obs_v == 1:
                ...
                obs_size = len(self._track_bodies) * self._num_traj_samples * 15 # no difference. 
    ...
    def get_task_obs_size_detail(self):
        ...
        if "im_obs_left_hand_idxes" in self.__dict__:
            task_obs_detail['self_obs_left_hand_idxes'] = self.self_obs_left_hand_idxes
            task_obs_detail['self_obs_right_hand_idxes'] = self.self_obs_right_hand_idxes
            task_obs_detail['im_obs_left_hand_idxes'] = self.im_obs_left_hand_idxes
            task_obs_detail['im_obs_right_hand_idxes'] = self.im_obs_right_hand_idxes
            
            task_obs_detail['dof_names'] = self._dof_names
            task_obs_detail['left_hand_names'] = self.left_hand_names
            task_obs_detail['right_hand_names'] = self.right_hand_names
    ...
    def _compute_task_obs(self, env_ids=None, save_buffer = True):
        ...
        if self.obs_v == 1 :
        ...
        elif self.obs_v == 4 or self.obs_v == 5 or self.obs_v == 6 or self.obs_v == 8 or self.obs_v == 9 or self.obs_v == 10:
            ...
            if self.obs_v == 4 or self.obs_v == 6:
                ...
                obs = compute_imitation_observations_v10(root_pos, root_rot, ref_rb_pos_subset, ref_rb_rot_subset, ref_body_vel_subset, ref_body_ang_vel_subset, time_steps, self._has_upright_start)
    ...
    def _compute_reset(self):
        ...
        if self.zero_out_far and self.zero_out_far_train:
            ...
            # print(self._contact_forces.detach().numpy())
            # print(self._contact_forces.numpy(), self._contact_forces.abs().sum(dim=-1)[0].nonzero().squeeze())
    ...
    def _build_im_obs_part_idex(self):
        if self.humanoid_type in ["smplh", "smplx"]:
            if self.obs_v == 6 and (len(self._track_bodies) == len(self._body_names)):
                curr_idx = 0
                left_idxes = []
                right_idxes = []
                
                for idx in range(len(self._body_names)): # diff_local_body_pos_flat
                ...
                for idx in range(len(self._body_names)): # diff_local_body_rot_flat
                ...
                for idx in range(len(self._body_names)): # diff_local_vel
                    if idx in self.left_hand_indexes: left_idxes += [curr_idx, curr_idx + 1 , curr_idx + 2]
                    elif idx in self.right_hand_indexes: right_idxes += [curr_idx, curr_idx + 1 , curr_idx + 2]
                    curr_idx += 3
                    
                for idx in range(len(self._body_names)): # diff_local_ang_vel
                    if idx in self.left_hand_indexes: left_idxes += [curr_idx, curr_idx + 1 , curr_idx + 2]
                    elif idx in self.right_hand_indexes: right_idxes += [curr_idx, curr_idx + 1 , curr_idx + 2]
                    curr_idx += 3
                
                for idx in range(len(self._body_names)): # local_ref_body_pos
                    if idx in self.left_hand_indexes: left_idxes += [curr_idx, curr_idx + 1 , curr_idx + 2]
                    elif idx in self.right_hand_indexes: right_idxes += [curr_idx, curr_idx + 1 , curr_idx + 2]
                    curr_idx += 3
                    
                for idx in range(len(self._body_names)): # local_ref_body_rot
                    if idx in self.left_hand_indexes: left_idxes += [curr_idx, curr_idx + 1 , curr_idx + 2, curr_idx + 3, curr_idx + 4, curr_idx + 5]
                    elif idx in self.right_hand_indexes: right_idxes += [curr_idx, curr_idx + 1 , curr_idx + 2, curr_idx + 3, curr_idx + 4, curr_idx + 5]
                    curr_idx += 6
                    
                self.im_obs_left_hand_idxes = np.array(left_idxes)
                self.im_obs_right_hand_idxes = np.array(right_idxes)
...
def compute_imitation_observations_v8(root_pos, root_rot, body_pos, body_rot, body_vel, body_ang_vel, ref_body_pos, ref_body_rot, ref_body_vel, ref_body_ang_vel, time_steps, upright):
    ...
    heading_rot_expand = heading_rot.unsqueeze(-2).repeat((1, J, 1))
...
def compute_imitation_observations_v9(root_pos, root_rot, body_pos, body_rot, body_vel, body_ang_vel, ref_body_pos, ref_body_rot, ref_root_vel, ref_body_root_ang_vel, time_steps, upright):
    # type: (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor,Tensor, Tensor,Tensor,Tensor, int, bool) -> Tensor
    # only has root linear velocity and angular velocity
    # Future tracks in this obs will not contain future diffs.
    obs = []
    B, J, _ = body_pos.shape
...
def compute_imitation_observations_v10(root_pos, root_rot, ref_body_pos, ref_body_rot, ref_body_vel, ref_body_ang_vel, time_steps, upright):
    ...
    obs.append(local_ref_body_pos.view(B, time_steps, -1))  # timestep  * 24 * 3
    obs.append(local_ref_body_rot.view(B, time_steps, -1))  # timestep  * 24 * 6
    obs.append(local_ref_body_vel.view(B, time_steps, -1))  # timestep  * 24 * 3
    obs.append(local_ref_body_ang_vel.view(B, time_steps, -1))  # timestep  * 24 * 3
