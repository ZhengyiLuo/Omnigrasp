    ...
    class Network(AMPBuilder.Network):
        ...
        def form_embedding(self, task_out_z, obs_dict = None):
            ...
            if self.z_type == 'vae':
                ...
                # task_out_left_z = self.z_left_mlp(torch.cat([self_obs[:, self.self_obs_left_hand_idxes], task_obs[:, self.im_obs_left_hand_idxes]], dim = -1))
                # vae_left_mu = self.z_left_mu(task_out_left_z)
                # vae_left_log_var = self.z_left_logvar(task_out_left_z)
                
                # task_out_right_z = self.z_right_mlp(torch.cat([self_obs[:, self.self_obs_right_hand_idxes], task_obs[:, self.im_obs_right_hand_idxes]], dim = -1))
                # vae_right_mu = self.z_right_mu(task_out_right_z)
                # vae_right_log_var = self.z_right_logvar(task_out_right_z)
                # vae_mu = torch.cat([vae_mu, vae_left_mu, vae_right_mu], dim = -1)
                # vae_log_var = torch.cat([vae_log_var, vae_left_log_var, vae_right_log_var], dim = -1)
