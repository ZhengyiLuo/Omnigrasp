...
def load_z_left_decoder(checkpoint, activation = "relu", z_type = "sphere", device = "cpu"):
    actvation_func = torch_utils.activation_facotry(activation)
    key_name = "a2c_network.actor_left_mlp"
    loading_keys = [k for k in checkpoint['model'].keys() if k.startswith(key_name)] 
    ...
    net_dict.decoder= actor
    if z_type == "vae" and "a2c_network.z_left_prior.0.weight" in checkpoint['model'].keys():
        prior_loading_keys = [k for k in checkpoint['model'].keys() if k.startswith("a2c_network.z_left_prior.")]
        z_prior = load_mlp(prior_loading_keys, checkpoint, actvation_func)
        z_prior.append(actvation_func())
        z_prior_mu = load_linear('a2c_network.z_left_prior_mu', checkpoint=checkpoint)
        
        z_prior.eval(); z_prior_mu.eval()
        net_dict.z_prior = z_prior.to(device)
        net_dict.z_prior_mu = z_prior_mu.to(device)

        if "a2c_network.z_left_prior_logvar.weight" in checkpoint['model'].keys():
            z_prior_logvar = load_linear('a2c_network.z_left_prior_logvar', checkpoint=checkpoint)
            z_prior_logvar.eval()
            net_dict.z_prior_logvar = z_prior_logvar.to(device)
    
    return net_dict

def load_z_right_decoder(checkpoint, activation = "relu", z_type = "sphere", device = "cpu"):
    actvation_func = torch_utils.activation_facotry(activation)
    key_name = "a2c_network.actor_right_mlp"
    loading_keys = [k for k in checkpoint['model'].keys() if k.startswith(key_name)] 
    ...
    net_dict.decoder= actor
    if z_type == "vae" and "a2c_network.z_right_prior.0.weight" in checkpoint['model'].keys():
        prior_loading_keys = [k for k in checkpoint['model'].keys() if k.startswith("a2c_network.z_right_prior.")]
        z_prior = load_mlp(prior_loading_keys, checkpoint, actvation_func)
        z_prior.append(actvation_func())
        z_prior_mu = load_linear('a2c_network.z_right_prior_mu', checkpoint=checkpoint)
        
        z_prior.eval(); z_prior_mu.eval()
        net_dict.z_prior = z_prior.to(device)
        net_dict.z_prior_mu = z_prior_mu.to(device)

        if "a2c_network.z_right_prior_logvar.weight" in checkpoint['model'].keys():
            z_prior_logvar = load_linear('a2c_network.z_right_prior_logvar', checkpoint=checkpoint)
            z_prior_logvar.eval()
            net_dict.z_prior_logvar = z_prior_logvar.to(device)
