...
def geodesic_loss(predicted_rotation_matrices, true_rotation_matrices):
    relative_rotations = torch.matmul(predicted_rotation_matrices, true_rotation_matrices.transpose(-2, -1))
    trace = torch.diagonal(relative_rotations, dim1=-2, dim2=-1).sum(-1)
    angles = torch.acos(torch.clamp((trace - 1) / 2, min=-1, max=1))
    return angles.mean()


def quaternion_loss(predicted_quaternions, true_quaternions):
    # Ensure quaternions are in the same hemisphere
    dot_product = (predicted_quaternions * true_quaternions).sum(dim=1)
    predicted_quaternions = predicted_quaternions * dot_product.sign().unsqueeze(-1)
    
    # Compute the L2 norm
    loss = torch.nn.functional.mse_loss(predicted_quaternions, true_quaternions, reduction='mean')
    return loss
...
class IMAmpAgent(amp_agent.AMPAgent):
    ...
    def optimize_kin(self, batch_dict):
        ...
        if self.config.get("learn_world", False):
            ...
            kin_dict = {k:v.view(self.minibatch_size // self.horizon_length, self.horizon_length, -1) for k, v in kin_dict.items()} # move kinemaitc data to sequential format
            self_obs_size = humanoid_env.get_self_obs_size() 
            self_obs_orig = batch_dict['obs_orig'][..., :self_obs_size].view(self.minibatch_size // self.horizon_length, self.horizon_length, -1)
            kin_dict['self_obs_and_action'] = torch.cat([self_obs_orig, kin_dict['dof_pos'], kin_dict['dof_vel'], kin_dict['actions']], dim=-1)
            next_state = humanoid_env.world_model.forward(kin_dict)

            B, T, J, _ = next_state.body_pos.shape
            
            rg_pos_next_pred = next_state.body_pos[:, :-1].reshape(-1, 3)
            rg_rot_next_pred = next_state.body_quat[:, :-1].reshape(-1, 4)
            rg_vel_next_pred = next_state.lin_vel[:, :-1].reshape(-1, 3)
            rg_ang_vel_next_pred = next_state.ang_vel[:, :-1].reshape(-1, 3)
            dof_pos_next_pred = next_state.dof_pos[:, :-1].reshape(-1, 3)
            dof_vel_next_pred = next_state.dof_vel[:, :-1].reshape(-1, 3)
            
            rg_pos_next_sim = kin_dict["rg_pos"][:, 1:].reshape(-1, 3)
            rg_rot_next_sim = kin_dict["rg_rot"][:, 1:].reshape(-1, 4)
            rg_vel_next_sim = kin_dict["rg_vel"][:, 1:].reshape(-1, 3)
            rg_ang_vel_next_sim = kin_dict["rg_ang_vel"][:, 1:].reshape(-1, 3)
            dof_pos_next_sim = kin_dict["dof_pos"][:, 1:].reshape(-1, 3)
            dof_vel_next_sim = kin_dict["dof_vel"][:, 1:].reshape(-1, 3)
            
            loss_body_rot = quaternion_loss(rg_rot_next_pred, rg_rot_next_sim) 
            loss_body_pos = (rg_pos_next_sim - rg_pos_next_pred).pow(2).mean() 
            loss_body_vel = (rg_vel_next_sim - rg_vel_next_pred).pow(2).mean()
            loss_body_ang_vel = (rg_ang_vel_next_sim - rg_ang_vel_next_pred).pow(2).mean()
            # loss_dof_pos = (dof_pos_next_sim - dof_pos_next_pred).pow(2).mean() # this triggers Nan due to torch_utils.quat_to_exp_map
            loss_dof_vel = (dof_vel_next_sim - dof_vel_next_pred).pow(2).mean()
            
            kin_loss = loss_body_rot + loss_body_pos  + loss_body_vel  + loss_body_ang_vel  + loss_dof_vel # + loss_dof_pos 
            self.kin_optimizer.zero_grad()
            kin_loss.backward()
            nn.utils.clip_grad_norm_(humanoid_env.world_model.parameters(), self.grad_norm)
            self.kin_optimizer.step()
            info_dict.update({"kin_loss": kin_loss, "kin_body_rot": loss_body_rot * 100, "kin_body_pos": loss_body_pos * 1000 , "kin_body_vel": loss_body_vel , "kin_body_ang_vel": loss_body_ang_vel , "kin_dof_vel": loss_dof_vel }) # "kin_dof_pos": loss_dof_pos , 
            return info_dict
        else:
            return super().optimize_kin(batch_dict)
    ...
    def run_eval_loop(self):
        ...
        humanoid_env._termination_distances[:] = humanoid_env.cfg.env.get("test_termination_distances", 0.5)  # if not humanoid_env.strict_eval else 0.25 # ZL: use UHC's termination distance
...
class OmnigraspAmpAgent(IMAmpAgent):
    def __init__(self, base_name, config):
        super().__init__(base_name, config)
        self.gt_rot, self.gt_rot_all = [], []
        self.pred_rot, self.pred_rot_all = [], []
    ...
    def eval(self):
        humanoid_env = self.vec_env.env.task
        humanoid_env.grab_termination_disatnce = torch.tensor([humanoid_env.cfg.env.test_termination_distances]).to(humanoid_env.grab_termination_disatnce) 
        info = self.run_eval_loop()
        humanoid_env.grab_termination_disatnce = torch.tensor([humanoid_env.cfg.env.grab_termination_distance]).to(humanoid_env.grab_termination_disatnce) # 
        self.update_training_data(info['failed_keys'])
        self.vec_env.env.task.recreate_sim(info['failed_keys'], self.epoch_num)
        del self.terminate_state, self.terminate_memory, self.mpjpe, self.mpjpe_all
        return info["eval_info"]
    ...
    def _post_step_eval(self, info, done):
        ...
        if self.curr_stpes >= curr_max or self.terminate_state.sum() == humanoid_env.num_envs:
            ...
            if (humanoid_env.start_idx + humanoid_env.num_envs >= humanoid_env._motion_lib._num_unique_motions):
                ...
                per_obj_succ_rate = {k: 1 - v for k, v in per_obj_fail_rate.items()}
                print("------------------------------------------")
                print(f"Success Rate: {self.success_rate:.10f}")
                print("All: ", " \t".join([f"{k}: {v:.3f}" for k, v in metrics_all_print.items()]))
                print("Succ: "," \t".join([f"{k}: {v:.3f}" for k, v in metrics_succ_print.items()]))
                print("Obj_succ: "," \t".join([f"{k}: {v:.3f}" for k, v in per_obj_succ_rate.items()]))
                print("Failed keys: ", len(failed_keys), failed_keys)
                ...
                eval_info = {
                    ...
                    "accel_dist": metrics_succ_print['accel_dist'], 
                    "vel_dist": metrics_succ_print['vel_dist'], 
                    "TTR": metrics_succ_print['TTR'],
                    "rot_error_succ": metrics_succ_print['rot_error'],
                }
                if len(per_obj_succ_rate) < 100:
                    eval_info.update({f"Obj.{k}": v for k, v in per_obj_succ_rate.items()})
                # failed_keys = humanoid_env._motion_lib._motion_data_keys[terminate_hist[:humanoid_env._motion_lib._num_unique_motions]]
                # success_keys = humanoid_env._motion_lib._motion_data_keys[~terminate_hist[:humanoid_env._motion_lib._num_unique_motions]]
                # print("failed", humanoid_env._motion_lib._motion_data_keys[np.concatenate(self.terminate_memory)[:humanoid_env._motion_lib._num_unique_motions]])
                # joblib.dump(failed_keys, "output/dgx/smpl_im_shape_long_1/failed_1.pkl")
